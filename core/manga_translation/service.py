...
class MangaTranslationService:
    ...
    def translate_manga_file(self, manga_path: str, target_language: str) -> Optional[str]:
        ...
        """
        log.info(f"开始同步翻译 '{manga_path}' 到 '{target_language}'。")
        ...
        try:
            ...
            if num_pages == 0:
                log.warning(f"数据源 {manga_path} 报告有 0 页。正在中止。")
                return None

            log.info(f"为任务 '{task_id}' 创建了包含 {num_pages} 页的数据源。")
            ...
            # 2. 处理页面（OCR、翻译）
            log.info(f"开始为任务 '{task_id}' 中的 {len(image_arrays)} 张图像进行页面处理。")
            # 这里使用 asyncio.run 是因为底层的处理器可能仍然是异步的
            manga_title = Path(manga_path).stem
            log.info(f"为 '{manga_title}' 构建翻译上下文。")
            ...
            if not translated_arrays:
                log.error(f"Page processing for '{task_id}' resulted in no translated pages.")
                return None

            # 3. 缓存结果
            log.info(f"页面处理完成。正在为任务 '{task_id}' 缓存 {len(translated_arrays)} 个结果。")
            ...
            log.info(f"正在为任务 '{task_id}' 将 {len(translated_arrays)} 页打包到 {temp_zip_file.name}")
            ...
            with zipfile.ZipFile(temp_zip_file.name, 'w', zipfile.ZIP_DEFLATED) as zf:
                for i, image_array in enumerate(translated_arrays):
                    ...
                    if success:
                        ...
                        log.warning(f"为任务 '{task_id}' 将页面 {i} 编码为 WebP 格式失败。")
            
            log.info(f"成功为任务 '{task_id}' 创建翻译后的归档文件于: {temp_zip_file.name}")
            return temp_zip_file.name

        except Exception as e:
            log.error(f"Error in synchronous translation workflow for task '{task_id}': {e}", exc_info=True)
            return None
        finally:
            self.processor.reset()
