...
class MangaInfo:
    ...
    def _parse_metadata(self):
        ...
        while True:
            ...
            # 改进汉化标签识别
            if any(keyword in tag_content for keyword in ['中国翻訳', '中国翻译', '中國翻譯', '中國翻訳']):
                self.tags.add('汉化:中国翻译')
            elif any(keyword in tag_content for keyword in ['汉化', '漢化', '翻訳', '翻译', '翻譯']):
...
class MangaLoader:
    ...
    def _add_to_cache(self, page_index, image):
        ...
        with self._cache_lock:
            ...
            # 释放空间直到足够
            while self._current_cache_size + img_size > self._cache_size_limit and self._image_cache:
    ...
    def _get_page_image_from_zip(manga, page_index):
        ...
        if not manga or not os.path.exists(manga.file_path):
            log.warning(f"无效的漫画对象或文件不存在: {getattr(manga, 'file_path', None)}")
            return None
