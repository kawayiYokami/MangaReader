...
// ==================== 主题切换 ====================
    async onThemeChange(theme) {
        ...
        // 2. 检查是否为 PyWebView 应用
        if (this.isPyWebView) {
            // 3. 只有 PyWebView 应用才向服务器发送保存请求
            try {
    ...
    onTagClick(tag, mangaToFocus) {
        const index = this.filterOptions.tags.indexOf(tag);
        if (index > -1) {
        ...
        // 如果传递了 mangaToFocus，则滚动到该卡片
        if (mangaToFocus) {
            this.$nextTick(() => {
                // 在移动端，DOM的更新可能需要更长的时间。
                // 添加一个小的延迟以确保元素在执行滚动前已完全渲染。
                setTimeout(() => {
                    console.log('[Debug] Attempting to find and scroll to card...');
                    // 对文件路径中的反斜杠进行转义，以确保选择器在Windows上也能正常工作
                    const escapedPath = mangaToFocus.file_path.replace(/\\/g, '\\\\');
                    const selector = `.manga-card-mobile[data-manga-path="${escapedPath}"]`;
                    console.log(`[Debug] Selector: ${selector}`);
                    const cardElement = document.querySelector(selector);
                    console.log(`[Debug] Element Found:`, cardElement);
                    ...
                    if (cardElement) {
                        ...
                        console.log('[Debug] scrollIntoView called.');
                    }
                }, 100); // 稍微增加延迟以进行测试
            });
        }
        ...
        try {
            ...
            if (stateStr) {
                ...
                if (Date.now() - state.timestamp < 24 * 60 * 60 * 1000) {
                    ...
                    this.searchQuery = ''; // Do not restore search query
                    this.filterOptions.tags = state.filterOptions?.tags || state.selectedTags || [];
    ...
    initThumbnailObserver() {
        this.$nextTick(() => {
                ...
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        if (mangaPath && !this.thumbnailCache.has(mangaPath) && !this.loadingThumbnails.has(mangaPath)) {
                            // isPreload = false, as it's now in viewport or pre-load margin
            ...
            console.log('🔍 缩略图懒加载Observer已在DOM更新后初始化');
    ...
    observeAllCards() {
        this.$nextTick(() => {
            const cards = document.querySelectorAll('.manga-card-mobile');
            cards.forEach(card => {
                this.observeCard(card);
            });
            console.log(`👀 正在观察 ${cards.length} 个漫画卡片`);
        });
    },

    async loadThumbnail(mangaPath, isPreload = false) {
        // console.log(`[Thumbnail] 触发加载: ${mangaPath}, 预加载: ${isPreload}`);
        if (this.loadingThumbnails.has(mangaPath) || this.thumbnailCache.has(mangaPath)) {
            return;
        }
        ...
        try {
            ...
            if (thumbnailUrl) {
                ...
                // 如果加载失败，可以设置一个占位符或错误状态
                // this.thumbnailCache.set(mangaPath, 'path/to/error-image.png');
            }
        } catch (error) {
            console.error('获取缩略图失败:', mangaPath, error);
        } finally {
            this.loadingThumbnails.delete(mangaPath);
            // 再次强制UI更新以显示加载的图片或错误状态
            this.$forceUpdate();
        }
    },
    ...
    getOtherTags(tags) {
        ...
        for (const tag of tags) {
            ...
            let displayTag = tag;
            if (tag.includes(':')) {
                displayTag = tag.substring(tag.indexOf(':') + 1);
            }
            result.push({
                full: tag,
                display: displayTag
    ...
    displayAllTagsModal(manga) {
        ...
        if (allTags.length > 0) {
            message += `标签: ${allTags.map(t => t.display).join(', ')}`;
        }
    ...
    initInfiniteScrollObserver() {
        this.$nextTick(() => {
            ...
            if (!trigger) {
                // 在非随机模式下，或列表为空时，触发器可能不存在，这是正常现象
                return;
            }
            ...
            this.infiniteScrollObserver = new IntersectionObserver((entries) => {
                if (entries[0].isIntersecting && this.isRandomMode && !this.isLoadingNextPage) {
                    this.loadRandomManga(true);
                }
            }, {
                root: null, // 使用浏览器视口
                threshold: 0.1
    ...
    async toggleRandomMode() {
        ...
        if (this.isRandomMode) {
            // 进入随机模式
            this.searchQuery = ''; // 清空搜索和筛选
            this.filterOptions.tags = [];
            await this.loadRandomManga(false); // 加载第一页
            this.initInfiniteScrollObserver(); // 启动观察者
        } else {
            // 退出随机模式
            this.disconnectInfiniteScrollObserver(); // 停止观察者
            this.randomSessionId = null;
            this.currentRandomPage = 1;
            // 重新加载常规列表
            await this.loadMangaData();
        }
    },

    async loadRandomManga(append = false) {
        if (!append) {
            // 加载第一页
            this.isLoading = true;
            this.currentRandomPage = 1;
            this.randomSessionId = null;
            try {
                const response = await axios.post('/api/manga/random-session', { limit: 50 });
                if (response.data.success) {
                    this.mangaList = response.data.manga_list;
                    this.randomSessionId = response.data.session_id;
                    ElMessage.success('已进入随机阅读模式');
                } else {
                    ElMessage.error(response.data.message || '开启随机模式失败');
                    this.isRandomMode = false; // 如果失败，则切回普通模式
                }
            } catch (error) {
                ElMessage.error('请求随机漫画失败');
                console.error(error);
            ...
            // 加载下一页
            if (!this.randomSessionId || this.isLoadingNextPage) return;

            this.isLoadingNextPage = true;
            this.currentRandomPage++;
            try {
                const response = await axios.get(`/api/manga/random-session/${this.randomSessionId}`, {
                    params: {
                        page: this.currentRandomPage,
                        limit: 50
                    }
                });
                if (response.data.success && response.data.manga_list.length > 0) {
                    this.mangaList = [...this.mangaList, ...response.data.manga_list];
                } else {
                    // 没有更多数据了
                    this.currentRandomPage--; // 回滚页码
                    ElMessage.info('已加载所有随机漫画');
                }
            } catch (error) {
                console.error('加载下一页随机漫画失败:', error);
                this.currentRandomPage--; // 回滚页码
            } finally {
