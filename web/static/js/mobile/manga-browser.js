...
// ==================== 主题切换 ====================
    ...
    clearFilters() {
        this.searchQuery = '';
        this.filterOptions.tags = []; // 使用新的过滤数组
    },
    ...
    selectManga(manga) {
        ...
        const viewerUrl = `/dev/viewer-mobile?path=${encodedPath}&page=0`;
    ...
    saveBrowsingState() {
        const state = {
            ...
            selectedTags: [...this.filterOptions.tags],
        ...
        try {
            ...
            if (stateStr) {
                ...
                if (Date.now() - state.timestamp < 24 * 60 * 60 * 1000) {
                    ...
                    this.filterOptions.tags = state.selectedTags || [];
    ...
    getOtherTags(tags) {
        ...
        for (const fullTag of tags) {
            if (fullTag.startsWith('标题:')) {
                continue; // 跳过标题标签
            }
            
            let displayTag = fullTag;
            if (fullTag.includes(':')) {
                displayTag = fullTag.substring(fullTag.indexOf(':') + 1);
            }
    ...
    onTagClick(fullTag) {
        const index = this.filterOptions.tags.indexOf(fullTag);
        if (index > -1) {
            this.filterOptions.tags.splice(index, 1);
        } else {
            this.filterOptions.tags.push(fullTag);
        }
    },
